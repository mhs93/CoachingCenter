...
@section('content')
    ...
    @push('js')
        ...
        <script>
            ...
            // Get all data from database (Server Side)
            $(document).ready(function() {
                $('.data-table').DataTable({
                    processing: true,
                    serverSide: true,
                    info: true,
                    ajax: "{{ route('admin.routine.list') }}",
                    'pageLength': 10,
            ...
            function statusConfirm(id) {
                ...
                swal({
                    title: `Are you sure?`,
                    text: 'You want to change routine status ?',
                    buttons: true,
                    dangerMode: true,
            ...
            function showDeleteConfirm(id)
            {
                ...
                swal({
                    title: `Are you sure?`,
                    text: 'You want to delete this routine ?',
                    buttons: true,
                    dangerMode: true,
                }).then((willDelete) => {
                    if (willDelete) {
            ...
            {
                var url = '{{ route("admin.routine.destroy",":id") }}';
                $.ajax({
                    type: "DELETE",
                    url: url.replace(':id', id ),
